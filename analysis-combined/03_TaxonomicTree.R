...
count_per_genus <- function(physeq){
  ...
  # table(sample_sums(physeq.CSN)) # sanity check total of 100 counts per sample
...
p1 <- ggtree(tree.all, layout="circular", branch.length="none", aes(color=group), lwd=0.4)+
  # geom_tiplab(aes(label=new_label), size=2, color="black")+
  # geom_nodepoint(size=0.1, color="black")+
  scale_color_manual(values=colors)+
  labs(color="Phylum")

p2 <- gheatmap(p1, asvDF.all, offset=0.5, width=.1, colnames=FALSE)+
...
jpeg("~/Projects/IBS_Meta-analysis_16S/data/analysis-combined/03_TaxonomicTree/taxTree_all.jpg", width=5000, height=2500, res=400)
gheatmap(p2+new_scale_fill(),
         countDF.all, offset=1.2, width=.1, colnames = FALSE)+
...
p2_fecal <- gheatmap(p1_fecal, asvDF.fecal, offset=0.5, width=.1, colnames=FALSE)+
  scale_fill_gradient2(low = muted("red"),
                       mid = "white",
                       high = "#2b8cbe",
                       trans="log",
...
jpeg("~/Projects/IBS_Meta-analysis_16S/data/analysis-combined/03_TaxonomicTree/taxTree_fecal.jpg", width=5000, height=2500, res=400)
gheatmap(p2_fecal+new_scale_fill(),
         countDF.fecal, offset=1.2, width=.1, colnames = FALSE)+
  scale_fill_gradient(low = "white",
                      high = muted("red"),
                      trans="log",
                      breaks=c(1e-5,1e-3,1e-1,10),
...
# physeq.fecal.glom <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-combined/03_TaxonomicTree/physeq_sigmoid_glomGenus.rds")
...
p2_sigmoid <- gheatmap(p1_sigmoid, asvDF.sigmoid, offset=0.5, width=.1, colnames=FALSE)+
  scale_fill_gradient2(low = muted("red"),
                       mid = "white",
                       high = "#2b8cbe",
                       trans="log",
...
jpeg("~/Projects/IBS_Meta-analysis_16S/data/analysis-combined/03_TaxonomicTree/taxTree_sigmoid.jpg", width=5000, height=2500, res=400)
gheatmap(p2_sigmoid+new_scale_fill(),
         countDF.sigmoid, offset=1.2, width=.1, colnames = FALSE)+
  scale_fill_gradient(low = "white",
                      high = muted("red"),
                      trans="log",
                      breaks=c(1e-5,1e-3,1e-1,10),
