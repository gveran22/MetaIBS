...
path.data   <- file.path(path.root, "data/preprocessing/analysis-individual/Fukui-2020")
```
...
physeq            <- readRDS(file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/phylotree/input/physeq_fukui.rds")) # phyloseq object
```
...
path.fastq <- file.path(path.fukui, "download-Fukui-samples/")
...
### 4.1. Primer removal
The vast majority of the reads contain their respective primers (>95% of reads). After looking at the fastq files, the primers are located at the very beginning of the reads, as expected.
...
We will simply perform a quality filtering of the reads, and trim the first 25bp of the reads.
...
saveRDS(nb.reads, file.path(path.root, "data/preprocessing/dada2-filt/nbreads_fukui.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_fukui.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/taxonomy/output/taxa_fukui.rds"))
```
...
# Import metadata
metadata_table <- read.table(file.path(path.agp, "00_Metadata-Fukui/FukuiSraRunTable.txt"), header = TRUE, sep = ",")

# Keep relevant columns
metadata_table <- metadata_table[, c("Run", "Sample.Name", "Collection_date")]
...
# Sanity check
table(metadata_table$host_disease) # 85 IBS & 26 HC
table(metadata_table$host_subtype) # 85 IBS & 26 HC
...
saveRDS(physeq, file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/phylotree/input/physeq_fukui.rds"))
```
