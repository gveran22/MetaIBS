##########################
# Purpose: Add phylogenetic tree to Nagel phyloseq object
# Date: February 2023
# Author: Salomé Carcy
##########################
...
#____________________________________________________________________
# IMPORT DATA
physeq <- readRDS(file.path(path.root, "input/physeq_nagel.rds"))
...
taxa_names(physeq) <- paste0("ASV_nagel_", seq(ntaxa(physeq))) # replace the whole dna sequences in the taxa_names by a surname ASV_nagel_1, ASV_nagel_2, etc.

# Save physeq object
saveRDS(physeq, file.path(path.root, "output/physeq_nagel.rds"))
