...
knitr::opts_chunk$set(fig.width = 3.5, fig.height = 3, warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/MetaIBS/scripts/analysis-individual/Nagel-2016/")
```
...
trim_samples  <- sort(list.files(file.path(path.data, "raw_fastq_trimmed"), pattern=".fastq.gz", full.names = TRUE)) # reads with length >50bp
...
physeq        <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_nagel.rds")) # phyloseq object without phylogenetic tree
```
...
# saveRDS(nb.reads, file.path(path.root, "data/analysis-combined/06_QCplot/nbreads_nagel.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_nagel.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/output/taxa_nagel.rds"))
```
