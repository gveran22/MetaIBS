...
knitr::opts_chunk$set(fig.width = 15, fig.height = 5, warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/IBS_Meta-analysis_16S/analysis-individual/Zhu-2019/")
```
...
path.plots <- file.path(path, "data/analysis-individual/PLOTS/plots-Zhu-EDA")
...
# ggsave(file.path(path.plots, "absAbundance_phylum.jpg"), width=10, height=5)
...
# Save object
saveRDS(physeq.NZcomp, file.path(path, "data/analysis-individual/Zhu-2019/02_EDA-Zhu/physeq_NZcomp.rds"))
...
saveRDS(physeq.rel, file.path(path, "data/analysis-individual/Zhu-2019/02_EDA-Zhu/physeq_relative.rds"))
...
saveRDS(physeq.CSN, file.path(path, "data/analysis-individual/Zhu-2019/02_EDA-Zhu/physeq_CSN.rds"))
...
# save the physeq.rel object
saveRDS(physeq.clr, file.path(path, "data/analysis-individual/Zhu-2019/02_EDA-Zhu/physeq_clr.rds"))
```
...
```{r zhu-asv-anomaly, eval=FALSE, echo=FALSE}
# Order samples as healthy/IBS
sample_order <- data.frame(sample_data(physeq.zhu)) %>%
  arrange(host_disease)

# ****** PRE-CUT ASVs ******
# Plot ASVs abundance
plot_heatmap(physeq.CSN, sample.label="host_disease", method="MDS", distance="bray", low="#000033", high="#CCFF66",
             sample.order=rownames(sample_order)) +
  ggtitle("Heatmap on Zhu dataset with common scale normalization")+
  theme(axis.text.y = element_blank())
# ggsave(file.path(path.plots, "heatmap_ASVs.jpg"), height=6, width=8, dpi=300)
...
# Normalize
physeq.small.CSN <- transform_sample_counts(physeq.small, function(x) (x*min(sample_sums(physeq.small))) / sum(x) )
table(rowSums(otu_table(physeq.small.CSN))) # all samples sum to 8301 total count
# Plot ASVs abundance
plot_heatmap(physeq.small.CSN, sample.label="host_disease", method="MDS", distance="bray", low="#000033", high="#CCFF66",
             sample.order=rownames(sample_order)) +
  ggtitle("Heatmap on Zhu dataset with common scale normalization (post-cut ASVs)")+
  theme(axis.text.y = element_blank())
```
...
# ggsave(file.path(path.plots, "distances4_MDS(Family).jpg"), height = 4, width = 15)
```
...
```
![Alpha diversity of ace (C) and simpson (D) index of the IBS patients and the healthy controls](/Users/scarcy/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Zhu-2019/02_EDA-Zhu/fig4.png){width=60%}
