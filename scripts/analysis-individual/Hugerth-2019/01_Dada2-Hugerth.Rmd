...
FWDpos            <- readRDS(file.path(path.data, "01_Dada2-Hugerth/forwardreads_primerposition.rds"))
REVpos            <- readRDS(file.path(path.data, "01_Dada2-Hugerth/reversereads_primerposition.rds"))
...
FWD.filt1_samples <- sort(list.files(file.path(path.data, "filtered1"), pattern="_F_filt1.fastq.gz", full.names = TRUE)) # reads with primers removed
REV.filt1_samples <- sort(list.files(file.path(path.data, "filtered1"), pattern="_R_filt1.fastq.gz", full.names = TRUE)) # reads with primers removed
FWD.filt2_samples <- sort(list.files(file.path(path.data, "filtered2"), pattern="_F_filt2.fastq.gz", full.names = TRUE)) # quality-filtered reads
REV.filt2_samples <- sort(list.files(file.path(path.data, "filtered2"), pattern="_R_filt2.fastq.gz", full.names = TRUE)) # quality-filtered reads
...
```{r primer-position-plot, fig.width = 8, fig.height = 4}
...
nb.reads <- data.frame("after"=rowSums(seqtable.nochim)) %>%
  ...
  right_join(raw_stats %>%
               dplyr::rename(Run=sample, before=reads),
             by="Run", keep=F) %>%
  relocate(before) %>%
  relocate(Run) %>%
