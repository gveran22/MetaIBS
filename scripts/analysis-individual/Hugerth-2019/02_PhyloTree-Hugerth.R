##########################
# Purpose: Add phylogenetic tree to Hugerth phyloseq object
# Date: February 2023
# Author: Salomé Carcy
##########################
...
#____________________________________________________________________
# IMPORT DATA
physeq <- readRDS(file.path(path.root, "input/physeq_hugerth.rds"))
...
taxa_names(physeq) <- paste0("ASV_hugerth_", seq(ntaxa(physeq))) # replace the whole dna sequences in the taxa_names by a surname ASV_hugerth_1, ASV_hugerth_2, etc.

# Save physeq object
saveRDS(physeq, file.path(path.root, "output/physeq_fukui.rds"))
