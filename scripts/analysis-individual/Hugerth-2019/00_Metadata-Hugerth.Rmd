...
knitr::opts_chunk$set(warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/MetaIBS/scripts/analysis-individual/Hugerth-2019")
```
...
sra_metadata <- read.csv(file.path(path.data, "00_Metadata-Hugerth/HugerthSraRunTable.txt"), header=TRUE, sep = ",")
head(sra_metadata)

# Import the supplementary ASV table from the paper
paper_supp <- read_excel(file.path(path.data, "00_Metadata-Hugerth/ASV_table_PAPER.xlsx"))
head(paper_supp)

# Import the "suggested" runs from the paper (samples ERS3379832-ERS3380418)
paper_runs <- read.csv(file.path(path.data, "00_Metadata-Hugerth/paper_suggested.csv"), header=TRUE, sep = ",")
head(paper_runs)
```
...
paperDF <- paperDF %>%
  separate(col=raw_names, into=c("sequencing_run", "extraction_plate", "host_ID", "sample_type"), sep="_", remove=FALSE) %>%
  # rename columns
  # rename_with(tolower) %>%
...
# Looking into sra_metadata, it looks like
# subject 3967 has one biopsy (seqrun 170519, extraction plate 27-583) & one stool (seqrun 180406, extraction plate 27-2001) sample
# subject 4149 has one biopsy (seqrun 170519, extraction plate 27-583) & one stool (seqrun 180406, extraction plate 27-2001) sample
# subject 4206 has one biopsy (seqrun 170519, extraction plate 27-587) & one stool (seqrun 180406, extraction plate 27-2001) sample
# subject 4339 has one biopsy (seqrun 170519, extraction plate 27-587) & one stool (seqrun 180406, extraction plate 27-2001) sample
# so it makes sense that these 4 samples from the paper supp. ASV table don't have an equivalent into the SRA metadata
...
# => 322 healthy samples (paper describes 466)
# => 96 IBS samples (paper describes 95)
# => rest is non-IBS and non-healthy (n=144)
```


## 2.3. Samples to download as indicated in paper
...
Now, the _paper_suggested.csv_ file for samples ERS3379832-ERS3380418 was downloaded on the SRA database. Let's import it, and compare the Run numbers from that file, with the Run numbers we indirectly obtained from the supplementary ASV table.
...
paperRuns.DF <- paper_runs %>%
  ...
  dplyr::select(c(col, "Sample")) %>%
  filter(environment_.biome. != "bioreactor") %>% 
  dplyr::select(-environment_.biome.) %>%
  rename_with(tolower) %>%
  rename(Run=run,
         sample_type=host_body_product,
         host_bmi=host_body.mass_index,
...
# HOWEVER, these samples correspond to the same individuals!! (same host_IDs...)
```
...
*********************************************
# 3. IDENTIFY INCONSISTENCIES IN SRA METADATA
*********************************************
...
## 3.1. Inconsistencies in disease phenotype
...
## 3.3. Inconsistencies in psychological disorder
...
*******************************************
# 4. REMOVE INCONSISTENCIES IN SRA METADATA
*******************************************
...
<br/>
<br/>
![Schematic representation of the filtering process](/Users/enigma/Projects/MetaIBS/data/analysis-individual/Hugerth-2019/00_Metadata-Hugerth/Metadata_Strategy.jpeg)
<br/>
<br/>
...
clean.df <- metadata %>%
  # 1) STRINGENT FILTERING: group_by same BMI, age, disease, collection date & gender
  group_by(host_bmi, host_age, host_disease, coldate_y, coldate_m, host_sex) %>%
  mutate(ID_stringent=dplyr::cur_group_id()) %>%
  ungroup() %>%
  ...
  mutate(ID_flexibl=dplyr::cur_group_id()) %>%
  ungroup() %>%
...
For the sake of being rigorous/cautious, we will keep the **stringent** filtering option: samples with different gender will be considered as different individuals.
```{r}
# Finish cleaning up metadata
clean.df <- clean.df %>%
  # designate sex_stringent and ID_stringent as host_sex and host_ID
  rename(host_ID=ID_stringent,
         host_sex=sex_stringent) %>%
  ...
  relocate(Run) %>%
  relocate(c(host_ID,host_disease, host_subtype, sample_type, host_age, host_bmi, host_sex, host_psy, Collection, author, sequencing_tech, variable_region), .after=Run)

# Sanity checks
table(clean.df[,c("host_disease", "host_subtype")], useNA="ifany")
table(clean.df$sample_type, useNA="ifany")
table(clean.df$host_sex, useNA="ifany")
summary(clean.df$host_bmi)
summary(clean.df$host_age)
```
...
## 5.1. Get list of links to wget fastq files from ENA
```{r savedata1}
# Open the filereport downloaded from the ENA
ena_table <- read.csv(file.path(path.data, "raw_fastq/filereport_read_run_PRJEB31817_tsv.txt"), header=T, sep="\t")
table(ena_table$run_accession %in% clean.df$Run, useNA="ifany") # 607 samples of interest
ena_table <- ena_table[ena_table$run_accession %in% clean.df$Run,]
dim(ena_table) # 607 rows, like clean.df
head(ena_table)
...
## 5.2. Export metadata & fastq file links
```{r savedata2, echo=TRUE, eval=FALSE}
# Save the metadata
write.csv(clean.df, file.path(path.data, "00_Metadata-Hugerth/Metadata-Hugerth.csv"))

# Export the list of links to download the Runs from ENA
write.table(ena_table_final$fastq_ftp, file.path(path.data, "raw_fastq/list_files_hugerth.txt"),
