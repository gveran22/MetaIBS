...
path.metadata <- file.path(path.root, "scripts/analysis-individual/Hugerth-2019/00_Metadata-Hugerth")

# Metadata table downloaded from the SRA
sra_metadata <- read.csv(file.path(path.metadata, "HugerthSraRunTable.txt"), header=TRUE, sep = ",")
head(sra_metadata)

# Import the supplementary ASV table from the paper
paper_supp <- read_excel(file.path(path.metadata, "Paper_Samples/ASV_table_PAPER.xlsx"))
head(paper_supp)

# Import the "suggested" runs from the paper (samples ERS3379832-ERS3380418)
paper_runs <- read.csv(file.path(path.metadata, "paper_suggested.csv"), header=TRUE, sep = ",")
head(paper_runs)
```
...
<br/>
<br/>
![Schematic representation of the filtering process](./00_Metadata-Hugerth/Metadata_Strategy.jpeg)
<br/>
<br/>
...
# Save the metadata
# write.table(clean.df, file.path(path.root, "data_local/analysis-individual/Hugerth-2019/00_Metadata-Hugerth/modif_metadata(R).csv"))
write.table(clean.df, file.path(path.metadata, "Metadata-Hugerth.csv"))

# Export the list of Runs to download
write.table(clean.df$Run, file.path(path.root, "scripts/analysis-individual/Hugerth-2019/download-Hugerth-samples/list_files_hugerth.txt"),
