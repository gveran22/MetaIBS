...
path.zeber <- file.path(path.root, "data/preprocessing/analysis-individual/Zeber-2016/03_EDA-Zeber")

# Import phyloseq object
physeq.zeber <- readRDS(file.path(path.root, "data/phyloseq-objects/physeq_zeber.rds"))
...
physeq.rel    <- readRDS(file.path(path.zeber, "physeq_relative.rds"))
physeq.clr    <- readRDS(file.path(path.zeber, "physeq_clr.rds"))
physeq.CSN    <- readRDS(file.path(path.zeber, "physeq_CSN.rds"))
physeq.NZcomp <- readRDS(file.path(path.zeber, "physeq_NZcomp.rds"))
```
...
# ggsave(file.path(path.zeber, "absAbundance_phylum.jpg"), width=13, height=5)
...
# ggsave(file.path(path.zeber, "relAbundance_phylum.jpg"), width=12, height=5)
...
ggplot(class.table, aes(x = reorder(Sample, Sample, function(x) mean(class.table[Sample == x & Phylum == 'Firmicutes', 'Abundance'])),
  ...
  theme(axis.text.x = element_text(size=4, angle=90))+
  labs(x = "Samples", y = "Relative abundance", title = "Zeber-Lubecka dataset (2016)")
# ggsave(file.path(path.zeber, "relAbundance_class.jpg"), width=12, height=5)
```
...
# ggsave(file.path(path.zeber, "ratioFB.jpg"), width=4, height=6)
...
ggplot(ratio.FB, aes(x = host_subtype, y = logRatioFB))+
  ...
  theme_cowplot() +
  theme(legend.position="none")
# ggsave(file.path(path.zeber, "ratioFB_subtype.jpg"), width=5, height=7)
...
# Save object
saveRDS(physeq.NZcomp, file.path(path.zeber, "physeq_NZcomp.rds"))
...
saveRDS(physeq.rel, file.path(path.zeber, "physeq_relative.rds"))
...
saveRDS(physeq.CSN, file.path(path.zeber, "physeq_CSN.rds"))
...
# save the physeq.rel object
saveRDS(physeq.clr, file.path(path.zeber, "physeq_clr.rds"))
```
...
# ggsave(file.path(path.zeber, "distances4_MDS.jpg"), height = 7, width = 14)
```
...
### 7.1. Figure 1
The phylogenetic tree of bacteria detected in samples
...
```
![Figure 1. The phylogenetic tree of bacteria detected in samples. Only genera present in more than 1% of reads are shown. A](/Users/scarcy/Projects/MetaIBS/data/preprocessing/analysis-individual/Zeber-2016/03_EDA-Zeber/fig1.png){width=70%}
...
![Figure 2 Bacteroidetes/Firmicutes ratios in the healthy control (HC), IBS-C, IBS-D, and IBS-M groups.](/Users/scarcy/Projects/MetaIBS/data/preprocessing/analysis-individual/Zeber-2016/03_EDA-Zeber/fig2.png){width=50%}


### 7.3. Figure 4
Box plots of the Simpson index
...
plot_richness(physeq.zeber, x="host_disease", measures="Simpson", color="host_disease") +
  ...
  labs(x="", y="Simpson")
```
![Figure 4. Box plots of the Simpson index of community diversity in healthy controls (HCs) and IBS patients before (IBS) and after (Treat- ment) treatment.](/Users/scarcy/Projects/MetaIBS/data/preprocessing/analysis-individual/Zeber-2016/03_EDA-Zeber/fig4.png){width=50%}
