...
path.ringel <- file.path(path.root, "data/preprocessing/analysis-individual/Ringel-2015/03_EDA-Ringel")

# Import phyloseq object
physeq.ringel <- readRDS(file.path(path.root, "data/phyloseq-objects/physeq_ringel.rds"))
...
physeq.rel    <- readRDS(file.path(path.ringel, "physeq_relative.rds"))
physeq.clr    <- readRDS(file.path(path.ringel, "physeq_clr.rds"))
physeq.CSN    <- readRDS(file.path(path.ringel, "physeq_CSN.rds"))
physeq.NZcomp <- readRDS(file.path(path.ringel, "physeq_NZcomp.rds"))
```
...
This dataset has no covariates.
...
# ggsave(file.path(path.ringel, "absAbundance_phylum.jpg"), width=13, height=5)
```
...
# ggsave(file.path(path.ringel, "relAbundance_phylum.jpg"), width=10, height=5)
...
ggplot(class.table, aes(x = reorder(Sample, Sample, function(x) mean(phylum.table[Sample == x & Phylum == 'Firmicutes', 'Abundance'])),
  ...
  theme(axis.text.x = element_blank())+
  labs(x = "Samples", y = "Relative abundance", title = "Ringel-Kulka dataset")
# ggsave(file.path(path.ringel, "relAbundance_class.jpg"), width=12, height=5)
```
...
# Save object
saveRDS(physeq.NZcomp, file.path(path.ringel, "physeq_NZcomp.rds"))
...
saveRDS(physeq.rel, file.path(path.ringel, "physeq_relative.rds"))
...
saveRDS(physeq.CSN, file.path(path.ringel, "physeq_CSN.rds"))
...
# save the physeq.rel object
saveRDS(physeq.clr, file.path(path.ringel, "physeq_clr.rds"))
```
...
# ggsave(file.path(path.ringel, "distances4_MDS.jpg"), height = 4, width = 15)
```
