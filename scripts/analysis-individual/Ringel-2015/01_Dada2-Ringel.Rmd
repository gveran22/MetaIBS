...
knitr::opts_chunk$set(fig.width = 3.5, fig.height = 3, warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/MetaIBS/scripts/analysis-individual/Ringel-2015/")
```
...
path.ringel  <- file.path(path.root, "scripts/analysis-individual/Ringel-2015")
path.data    <- file.path(path.root, "data/analysis-individual/Ringel-2015")
```
...
raw_stats     <- readRDS(file.path(path.data, "01_Dada2-Ringel/raw_stats.rds")) # seq depth
FWDpos        <- readRDS(file.path(path.data, "01_Dada2-Ringel/forwardprimer_position.rds"))
REVpos        <- readRDS(file.path(path.data, "01_Dada2-Ringel/reverseprimer_position.rds"))
out1          <- readRDS(file.path(path.data, "01_Dada2-Ringel/out1.rds")) # out process (primer removal)
out2          <- readRDS(file.path(path.data, "01_Dada2-Ringel/out2.rds")) # out process (QC)
...
err           <- readRDS(file.path(path.data, "01_Dada2-Ringel/err.rds")) # error rates
seq_infered   <- readRDS(file.path(path.data, "01_Dada2-Ringel/seq_infered.rds")) # inferred ASVs
taxa          <- readRDS(file.path(path.data, "01_Dada2-Ringel/taxa_ringel.rds")) # taxa
physeq        <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_ringel.rds")) # phyloseq object without phylogenetic tree
```
...
```{r primer-position, echo=TRUE, eval=FALSE}
...
### 3.1. Primer removal
The reads indeed contain the forward primer (and about half of them the reverse primer). We will keep only reads containing the primers, and then trim the primers!
...
# saveRDS(nb.reads, file.path(path.root, "data/analysis-combined/06_QCplot/nbreads_ringel.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_ringel.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/output/taxa_ringel.rds"))
```
...
# Import metadata
metadata_table <- read.csv(file.path(path.data, "00_Metadata-Ringel/Metadata-Ringel.csv"), row.names = 1)
...
# physeq <- subset_taxa(physeq, Kingdom != "Eukaryota") # there are no eukaryota
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(raw_stats,   file.path(path.data, "01_Dada2-Ringel/raw_stats.rds"))
saveRDS(FWDpos,      file.path(path.data, "01_Dada2-Ringel/forwardprimer_position.rds"))
saveRDS(REVpos,      file.path(path.data, "01_Dada2-Ringel/reverseprimer_position.rds"))
saveRDS(out1,        file.path(path.data, "01_Dada2-Ringel/out1.rds"))
saveRDS(out2,        file.path(path.data, "01_Dada2-Ringel/out2.rds"))
saveRDS(err,         file.path(path.data, "01_Dada2-Ringel/err.rds"))
saveRDS(seq_infered, file.path(path.data, "01_Dada2-Ringel/seq_infered.rds"))

# Taxa & Phyloseq object
saveRDS(taxa,   file.path(path.data, "01_Dada2-Ringel/taxa_ringel.rds"))
saveRDS(physeq, file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_ringel.rds"))
saveRDS(physeq, file.path(path.root, "data/phyloseq-objects/phyloseq-without-phylotree/physeq_ringel.rds"))
```
