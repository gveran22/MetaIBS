##########################
# Purpose: Add phylogenetic tree to Zhuang phyloseq object
# Date: February 2023
# Author: Salomé Carcy
##########################
...
#____________________________________________________________________
# IMPORT DATA
physeq <- readRDS(file.path(path.root, "input/physeq_zhuang.rds"))
...
taxa_names(physeq) <- paste0("ASV_zhuang_", seq(ntaxa(physeq))) # replace the whole dna sequences in the taxa_names by a surname ASV_zhuang_1, ASV_zhuang_2, etc.

# Save physeq object
saveRDS(physeq, file.path(path.root, "output/physeq_zhuang.rds"))
