##########################
# Purpose: Add phylogenetic tree to Liu phyloseq object
# Date: February 2023
# Author: Salomé Carcy
##########################
...
#____________________________________________________________________
# IMPORT DATA
physeq <- readRDS(file.path(path.root, "input/physeq_liu.rds"))
...
taxa_names(physeq) <- paste0("ASV_liu_", seq(ntaxa(physeq))) # replace the whole dna sequences in the taxa_names by a surname ASV_liu_1, ASV_liu_2, etc.

# Save physeq object
saveRDS(physeq, file.path(path.root, "output/physeq_liu.rds"))
