...
## 1.2. Data
```{r import-data}
path <- "~/Projects/MetaIBS/scripts/analysis-individual/Liu-2020"

# Metadata table downloaded from the SRA
sra_metadata <- read.csv(file.path(path, "00_Metadata-Liu/LiuSraRunTable.txt"), header=TRUE, sep = ",")
head(sra_metadata)
```
...
write.csv(clean.df, file.path(path, "00_Metadata-Liu/modif_metadata(R).csv"))

# Export the list of Runs to download
write.table(clean.df$Run, file.path(path, "download-Liu-samples/list_files_liu.txt"),
