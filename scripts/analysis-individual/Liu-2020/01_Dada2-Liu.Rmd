...
REVpos        <- readRDS(file.path(path.data, "01_Dada2-Liu/reverseprimer_position.rds")) # dataframe with position of reverse primer in every .fastq file (every sample)
...
seq_infered   <- readRDS(file.path(path.data, "01_Dada2-Liu/seq_infered.rds")) # inferred ASVs
taxa          <- readRDS(file.path(path.data, "01_Dada2-Liu/taxa_liu.rds")) # taxa
physeq        <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_liu.rds")) # phyloseq object without phylogenetic tree
```
...
# hist(log10(raw_stats$reads), breaks=100)
```
...
Let's have a quick look at where the reverse complement of the reverse primer is positioned in the reads
...
# in how many reads (from all the fastq files) does the reverse primer start before 400bp?
table(REVpos$start < 400) # only ~1700 out of 6 million
```
...
We can see that the reads length has a bit reduced (by ~50bp), which is consistent with the reverse primer position at 400-430bp.
...
saveRDS(nb.reads, file.path(path.root, "data/analysis-combined/06_QCplot/nbreads_liu.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_liu.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/output/taxa_liu.rds"))
```
...
# Import metadata
metadata <- read.csv(file.path(path.data, "00_Metadata-Liu/Metadata-Liu.csv")) %>% select(-X)
...
saveRDS(REVpos,      file.path(path.data, "01_Dada2-Liu/reverseprimer_position.rds"))
...
saveRDS(physeq, file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_liu.rds"))
saveRDS(physeq, file.path(path.root, "data/phyloseq-objects/phyloseq-without-phylotree/physeq_liu.rds"))
```
