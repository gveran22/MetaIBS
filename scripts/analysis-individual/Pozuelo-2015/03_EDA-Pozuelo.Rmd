...
path.pozuelo <- file.path(path.root, "data/preprocessing/analysis-individual/Pozuelo-2015/03_Pozuelo-EDA")

# Import phyloseq object
physeq.pozuelo <- readRDS(file.path(path.root, "data/phyloseq-objects/physeq_pozuelo.rds"))
...
physeq.rel    <- readRDS(file.path(path.pozuelo, "physeq_relative.rds"))
physeq.clr    <- readRDS(file.path(path.pozuelo, "physeq_clr.rds"))
physeq.CSN    <- readRDS(file.path(path.pozuelo, "physeq_CSN.rds"))
physeq.NZcomp <- readRDS(file.path(path.pozuelo, "physeq_NZcomp.rds"))
```
...
plot_bar(physeq.pozuelo, fill = "Phylum") + facet_wrap("host_disease", scales="free") +
  ...
  ylim(0,80000)
# ggsave(file.path(path.pozuelo, "absAbundance_phylum.jpg"), width=13, height=5)
...
# ggsave(file.path(path.pozuelo, "relAbundance_phylum.jpg"), width=10, height=5)
...
ggplot(class.table, aes(x = reorder(Sample, Sample, function(x) mean(class.table[Sample == x & Phylum == 'Firmicutes', 'Abundance'])),
  ...
  theme(axis.text.x = element_blank())+
  labs(x = "Samples", y = "Relative abundance", title = "Pozuelo dataset (2015)")
# ggsave(file.path(path.pozuelo, "relAbundance_class.jpg"), width=12, height=5)
```
...
# ggsave(file.path(path.pozuelo, "relAbundance_phylum_collection-times.jpg"), width=19, height=7)
```
...
# ggsave(file.path(path.pozuelo, "ratioFB.jpg"), width=5, height=7)
...
ggplot(ratio.FB, aes(x = host_subtype, y = logRatioFB))+
  ...
  theme_cowplot() +
  theme(legend.position="none")
# ggsave(file.path(path.pozuelo, "ratioFB_subtype.jpg"), width=6, height=6)
...
ggplot(ratio.FB, aes(x = Collection, y = logRatioFB))+
  ...
  theme_cowplot()+
  theme(legend.position="none")
# ggsave(file.path(path.pozuelo, "ratioFB_collection-time.jpg"), width=5, height=7)
...
ggplot(ratio.FB, aes(x = host_disease, y = logRatioFB))+
  ...
  theme_cowplot()+
  theme(legend.position="none")
# ggsave(file.path(path.pozuelo, "ratioFB_collection-time2.jpg"), width=5, height=7)
```
...
# Save object
saveRDS(physeq.NZcomp, file.path(path.pozuelo, "physeq_NZcomp.rds"))
...
saveRDS(physeq.rel, file.path(path.pozuelo, "physeq_relative.rds"))
...
saveRDS(physeq.CSN, file.path(path.pozuelo, "physeq_CSN.rds"))
...
# save the physeq.rel object
saveRDS(physeq.clr, file.path(path.pozuelo, "physeq_clr.rds"))
```
...
#### 5.1.1. All samples (both time points)
...
# ggsave(file.path(path.pozuelo, "distances4_MDS_all.jpg"), height = 10, width = 15)
```


#### 5.1.2. Each time point separately
...
# ggsave(file.path(path.pozuelo, "distances4_MDS_1stCollection.jpg"), height = 7, width = 15)
...
# ggsave(file.path(path.pozuelo, "distances4_MDS_2ndCollection.jpg"), height = 7, width = 15)
```
...
### 7.1. Figure 1
Redundancy analysis on unweighted UniFrac data
...
### 7.2. Figure 2
Higher relative abundance of Bacteroidetes
...
### 7.3. Figure 3
Higher relative abundance of two bacterial families
...
### 7.4. Figure 4
Dysbiosis at the family and genus level in IBS subtypes
