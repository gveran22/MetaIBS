...
FWDpos        <- readRDS(file.path(path.data, "01_Dada2-LoPresti/forwardprimer_position.rds")) # dataframe with position of reverse primer in every .fastq file (every sample)
...
Let's have a quick look at where the forward primer is positioned in the reads
...
for(i in 1:length(FNs)){
  cat("SAMPLE", i)
  new <- primerHitsPosition(FWD.orients["Forward"], FNs[[i]])
...
# Import metadata
metadata <- read.csv(file.path(path.data, "00_Metadata-LoPresti/Metadata-LoPresti.csv"), row.names=1)
...
saveRDS(FWDpos,      file.path(path.data, "01_Dada2-LoPresti/forwardprimer_position.rds"))
...
write.csv(as.data.frame(sample_data(physeq)), file.path(path.data, "00_Metadata-LoPresti/Metadata-LoPresti-phyloseq.csv")) # metadata table
...
saveRDS(physeq, file.path(path.root, "data/phyloseq-objects/phyloseq-without-phylotree/physeq_lopresti.rds"))
```
