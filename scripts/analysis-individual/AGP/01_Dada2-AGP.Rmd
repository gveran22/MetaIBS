...
path.data <- file.path(path.root, "data/analysis-individual/AGP")
```
...
filt2_samples <- sort(list.files(file.path(path.data, "filtered2"), pattern="_filt2.fastq.gz", full.names = TRUE)) # quality-filtered reads
...
track         <- readRDS(file.path(path.data, "01_Dada2-AGP/track.rds"))
...
hist(raw_stats$reads, breaks=100)
```
...
sraDF <- read.csv(file.path(path.data, "00_Metadata-AGP/AGPSraRunTable.csv"))
metadata <- read.csv(file.path(path.data, "00_Metadata-AGP/Metadata-AGP.csv"), row.names=1)
table(sraDF[sraDF$Run %in% metadata$Run,"pcr_primers..exp."], useNA="ifany")
# There are two sets of primers:
...
REV <- "GGACTACNVGGGTWTCTAAT" # 806R primer sequence
...
### 3.1. Primer removal
The reads barely contain any primer. We will go forward directly with the quality filtering.
...
FNs_filt <- sort(list.files(file.path(path.data, "filtered2"), pattern="_filt2.fastq.gz", full.names = TRUE))
sample.names_filt <- sapply(strsplit(basename(FNs_filt), "_filt2.fastq.gz"), `[`, 1)
names(FNs_filt) <- sample.names_filt
length(FNs_filt) # 1240
```

50 samples had 0 reads post quality filter. About ~30 of them had a large number of reads but none of them passed the quality filter. Let's inspect quickly the quality profile of some of them.
```{r quality-filter-plots}
for (i in 196:199){ # printing 2 samples before the "abnormal samples", that actually passed the quality filter (as comparison)
  show(plotQualityProfile(FNs[i]))
}
```
...
# Iterate through the 1240 samples
...
```{r}
# Inspect the inferred sequence variants from sample 1:5
for (i in 1:5){
  print(seq_infered[[i]])
...
# We should have 1240 samples (1240 rows)
dim(seqtable)
...
```{r reads-filtered-sanity-check, eval=FALSE, echo=TRUE}
...
rownames(out2) <- sapply(strsplit(rownames(out2), ".fastq.gz"), `[`, 1)
track <- out2 %>%
  as.data.frame() %>%
...
```{r reads-filtered-sanity-check2}
...
```{r seqdepth, echo=TRUE, eval=FALSE, include=FALSE}
...
# saveRDS(nb.reads, file.path(path.root, "data/analysis-combined/06_QCplot/nbreads_agp.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_agp.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/output/taxa_agp.rds"))
```
...
shortASV_tokeep <- names( shortASV[!(shortASV %in% bloom)] ) # 9,278 ASVs
...
longASV_tokeep <- intersect(longASV_tokeep1, longASV_tokeep2) # 7,797 ASVs

# 3 - Combine and get the **sequences** to keep (not the names)
ASV_to_keep <- allASV[c(shortASV_tokeep, longASV_tokeep)] # 17,075 ASVs
...
physeq.filt <- prune_samples(sample_sums(physeq.filt)>=500, physeq.filt) # 17,075 ASVs and 1,191 samples
physeq.filt <- prune_taxa(taxa_sums(physeq.filt)>0, physeq.filt) # remove taxa absent in all samples
print(physeq.filt) # 1,191 samples and 17,056 ASVs
...
saveRDS(track,       file.path(path.data, "01_Dada2-AGP/track.rds"))
...
saveRDS(tax_table(physeq.filt), file.path(path.data, "01_Dada2-AGP/taxa_agp_without_bloom.rds"))
saveRDS(physeq.filt,            file.path(path.root, "data/analysis-individual/CLUSTER/phylotree/input/physeq_agp.rds"))
saveRDS(physeq.filt,            file.path(path.root, "data/phyloseq-objects/phyloseq-without-phylotree/physeq_agp.rds"))
```
