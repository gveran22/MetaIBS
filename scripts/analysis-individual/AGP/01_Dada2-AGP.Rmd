...
shortASV_tokeep <- names( shortASV[!(shortASV %in% bloom)] ) # 9,297 ASVs
...
longASV_tokeep <- intersect(longASV_tokeep1, longASV_tokeep2) # 7,800 ASVs

# 3 - Combine and get the **sequences** to keep (not the names)
ASV_to_keep <- allASV[c(shortASV_tokeep, longASV_tokeep)] # 17,097 ASVs
...
physeq.filt <- prune_samples(sample_sums(physeq.filt)>=500, physeq.filt) # 17,097 ASVs and 1,191 samples
physeq.filt <- prune_taxa(taxa_sums(physeq.filt)>0, physeq.filt) # remove taxa absent in all samples
print(physeq.filt) # 1,191 samples and 17,078 ASVs
