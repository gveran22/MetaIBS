...
knitr::opts_chunk$set(fig.width = 15, fig.height = 5, warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/MetaIBS/analysis-individual/Mars-2020/")
```
...
path.mars <- file.path(path.root, "data/preprocessing/analysis-individual/Mars-2020/03_EDA-Mars")

# Import phyloseq object
physeq.mars <- readRDS(file.path(path.root, "data/phyloseq-objects/physeq_mars.rds"))
...
physeq.rel    <- readRDS(file.path(path.mars, "physeq_relative.rds"))
physeq.clr    <- readRDS(file.path(path.mars, "physeq_clr.rds"))
physeq.CSN    <- readRDS(file.path(path.mars, "physeq_CSN.rds"))
physeq.NZcomp <- readRDS(file.path(path.mars, "physeq_NZcomp.rds"))
```
...
# ggsave(file.path(path.mars, "absAbundance_phylum.jpg"), width=13, height=5)
...
# ggsave(file.path(path.mars, "relAbundance_phylum.jpg"), width=10, height=5)
...
ggplot(class.table, aes(x = reorder(Sample, Sample, function(x) mean(phylum.table[Sample == x & Phylum == 'Firmicutes', 'Abundance'])),
  ...
  labs(x = "Samples", y = "Relative abundance", title = "Mars dataset (2020)")
# ggsave(file.path(path.mars, "relAbundance_class.jpg"), width=12, height=5)
...
# ggsave(file.path(path.mars, "ratioFB.jpg"), width=4, height=6)
# Statistical test
...
ggplot(ratio.FB, aes(x = Collection, y = logRatioFB))+
  ...
  theme_cowplot()+
  theme(legend.position="none")
# ggsave(file.path(path.mars, "ratioFB_paired-data.jpg"), width=4, height=6)
...
ggplot(ratio.FB, aes(x = host_subtype, y = logRatioFB))+
  ...
  theme_cowplot()+
  theme(legend.position="none")
# ggsave(file.path(path.mars, "ratioFB_subtype.jpg"), width=5, height=5)
...
# Save object
saveRDS(physeq.NZcomp, file.path(path.mars, "physeq_NZcomp.rds"))
...
saveRDS(physeq.rel, file.path(path.mars, "physeq_relative.rds"))
...
saveRDS(physeq.CSN, file.path(path.mars, "physeq_CSN.rds"))
...
# save the physeq.rel object
saveRDS(physeq.clr, file.path(path.mars, "physeq_clr.rds"))
```
...
#### 5.1.1. Samples from 1st collection time point (more samples)
...
# ggsave(file.path(path.mars, "distances4_MDS_1stcollection.jpg"), height = 4, width = 15)
```
...
# ggsave(file.path(path.mars, "distances4_MDS_all.jpg"), height = 10, width = 15)
```
...
### 7.1. Figure 1J
Comparison of the Bray-Curtis beta-diversity dispersion
