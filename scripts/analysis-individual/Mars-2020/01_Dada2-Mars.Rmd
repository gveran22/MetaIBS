...
knitr::opts_chunk$set(fig.width = 3.5, fig.height = 3, warning=FALSE, message=FALSE,
                      root.dir = "~/Projects/MetaIBS/scripts/analysis-individual/Mars-2020")
```
...
raw_stats         <- readRDS(file.path(path.data, "01_Dada2-Mars/raw_stats.rds")) # seq depth
FWDpos            <- readRDS(file.path(path.data, "01_Dada2-Mars/forwardreads_primerposition.rds"))
REVpos            <- readRDS(file.path(path.data, "01_Dada2-Mars/reversereads_primerposition.rds"))
FWD.out1          <- readRDS(file.path(path.data, "01_Dada2-Mars/FWD_out1.rds"))
REV.out1          <- readRDS(file.path(path.data, "01_Dada2-Mars/REV_out1.rds"))
out2              <- readRDS(file.path(path.data, "01_Dada2-Mars/out2.rds")) # out process (QC)
FWD.filt1_samples <- sort(list.files(file.path(path.data, "filtered1"), pattern="_1_filt.fastq.gz", full.names = TRUE)) # FWD reads
REV.filt1_samples <- sort(list.files(file.path(path.data, "filtered1"), pattern="_2_filt.fastq.gz", full.names = TRUE)) # REV reads
FWD.filt2_samples <- sort(list.files(file.path(path.data, "filtered2"), pattern="_F_filt2.fastq.gz", full.names = TRUE)) # FWD reads
REV.filt2_samples <- sort(list.files(file.path(path.data, "filtered2"), pattern="_R_filt2.fastq.gz", full.names = TRUE)) # REV reads
errF              <- readRDS(file.path(path.data, "01_Dada2-Mars/errF.rds")) # error rates F
errR              <- readRDS(file.path(path.data, "01_Dada2-Mars/errR.rds")) # error rates R
dadaFs            <- readRDS(file.path(path.data, "01_Dada2-Mars/infered_seq_F.rds")) # infered ASVs F
dadaRs            <- readRDS(file.path(path.data, "01_Dada2-Mars/infered_seq_R.rds")) # infered ASVs R
mergers           <- readRDS(file.path(path.data, "01_Dada2-Mars/mergers.rds")) # mergers
taxa              <- readRDS(file.path(path.data, "01_Dada2-Mars/taxa_mars.rds")) # taxa
physeq            <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/PhyloTree/input/physeq_mars.rds")) # phyloseq object without phylogenetic tree
```
...
PRJEB37924. Only sigmoid biopsy samples that were analyzed by 16S rRNA sequencing were downloaded (see _00_Metadata-Mars_).
...
saveRDS(nb.reads, file.path(path.root, "data/analysis-combined/06_QCplot/nbreads_mars.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_mars.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/analysis-individual/CLUSTER/taxonomy/output/taxa_mars.rds"))
```
