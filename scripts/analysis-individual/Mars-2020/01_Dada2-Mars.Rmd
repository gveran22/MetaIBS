...
path.data <- file.path(path.root, "data/preprocessing/analysis-individual/Mars-2020")
```
...
physeq            <- readRDS(file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/PhyloTree/input/physeq_mars.rds")) # phyloseq object without phylogenetic tree
```
...
path.fastq <- file.path(path.mars, "download-Mars-samples/")
...
seqtable.new <- seqtable[,nchar(colnames(seqtable)) %in% 200:310]
dim(seqtable.new)
...
saveRDS(nb.reads, file.path(path.root, "data/preprocessing/dada2-filt/nbreads_mars.rds"))
```
...
# saveRDS(seqtable.nochim, file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/taxonomy/input/seqtablenochim_mars.rds"))
...
# taxa <- readRDS(file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/taxonomy/output/taxa_mars.rds"))
```
...
# Import metadata
metadata_table <- read.csv(file.path(path.mars, "00_Metadata-Mars/modif_metadata(R).csv")) %>% select(-X)
...
saveRDS(physeq, file.path(path.root, "data/preprocessing/analysis-individual/CLUSTER/PhyloTree/input/physeq_mars.rds"))
```
