...
path.labus  <- file.path(path.root, "Analysis/Scripts/Individual/Labus-2017")
path.data <- file.path(path.root, "data/phyloseq-objects")
path.plots <- file.path(path.root, "Analysis/Plots/Individual/Labus-2017")
```


## Read data
```{r read-data, echo=FALSE}
physeq <- readRDS(file.path(path.data, "physeq_labus.rds")) # phyloseq object
```
...
wilcox.test(ratio.FB[ratio.FB$host_disease == "IBS","logRatioFB"],
            ratio.FB[ratio.FB$host_disease == "Healthy","logRatioFB"]) # p = 0.03
