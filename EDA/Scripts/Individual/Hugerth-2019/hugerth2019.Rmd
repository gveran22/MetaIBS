...
path.hugerth  <- file.path(path.root, "Analysis/Scripts/Individual/Hugerth-2019")
path.data <- file.path(path.root, "data/phyloseq-objects")
path.plots <- file.path(path.root, "Analysis/Plots/Individual/Hugerth-2019")
```


## Read data
```{r read-data, echo=FALSE}
physeq <- readRDS(file.path(path.data, "physeq_hugerth.rds")) # phyloseq object
```
...
```{r rarefaction-curve-stool, echo=FALSE}
...
set.seed(1024)
rareres <- get_rarecurve(obj=physeq%>%ps_filter(sample_type=="stool"), chunks=400) # chunks: number of subsample in a sample
...
```{r rarefaction-curve-sig, echo=FALSE}

# number of species against the number of samples
# This curve is created by randomly re-sampling the pool of N samples several times and then plotting the average number of species found on each sample

set.seed(1024)
rareres <- get_rarecurve(obj=physeq%>%ps_filter(sample_type=="sigmoid"), chunks=400) # chunks: number of subsample in a sample
...
```{r alpha-diversity-stool, echo=FALSE}
...
alphaobj <- get_alphaindex(physeq%>%ps_filter(sample_type=="stool"))
...
```{r alpha-diversity-sig, echo=FALSE}

#   measure of microbiome diversity applicable to a single sample
#  for alpha diversity, many indices exist, each reflecting different aspects of community heterogeneity

alphaobj <- get_alphaindex(physeq%>%ps_filter(sample_type=="sigmoid"))
...
### Stool Samples
```{r pca-analysis-stool, fig.width = 10, fig.height = 4}

# If the input was normalized, the method parameter should be settled NULL.
pcares <- get_pca(obj=physeq%>%ps_filter(sample_type=="stool"), method="hellinger")
...
```{r pca-analysis-sig, fig.width = 10, fig.height = 4}

# If the input was normalized, the method parameter should be settled NULL.
pcares <- get_pca(obj=physeq%>%ps_filter(sample_type=="sigmoid"), method="hellinger")
...
```{r pcoa-analysis-stool, fig.width = 10, fig.height = 4}

# distmethod
# "unifrac",  "wunifrac", "manhattan", "euclidean", "canberra", "bray", "kulczynski" ...(vegdist, dist)
pcoares <- get_pcoa(obj=physeq%>%ps_filter(sample_type=="stool"), distmethod="bray", method="hellinger")
...
```{r pcoa-analysis-sig, fig.width = 10, fig.height = 4}

# distmethod
# "unifrac",  "wunifrac", "manhattan", "euclidean", "canberra", "bray", "kulczynski" ...(vegdist, dist)
pcoares <- get_pcoa(obj=physeq%>%ps_filter(sample_type=="sigmoid"), distmethod="bray", method="hellinger")
...
## 6.3. Permuatational Multivariate Analysis of Variance 

### Stool Samples

```{r perm-multi-analysis-stool, echo=FALSE}

distme <- get_dist(physeq%>%ps_filter(sample_type=="stool"), distmethod ="bray", method="hellinger")
sampleda <- data.frame(sample_data(physeq%>%ps_filter(sample_type=="stool")), check.names=FALSE)
...
```{r perm-multi-analysis-sig, echo=FALSE}

distme <- get_dist(physeq%>%ps_filter(sample_type=="sigmoid"), distmethod ="bray", method="hellinger")
sampleda <- data.frame(sample_data(physeq%>%ps_filter(sample_type=="sigmoid")), check.names=FALSE)
sampleda <- sampleda[match(colnames(as.matrix(distme)),rownames(sampleda)),,drop=FALSE]
sampleda$host_disease <- factor(sampleda$host_disease)
set.seed(1024)
adores <- adonis(distme ~ host_disease, data=sampleda, permutation=9999)
...
```{r hierarchical-cluster-stool, fig.width = 10, fig.height = 4}

hcsample <- get_clust(obj=physeq%>%ps_filter(sample_type=="stool"), distmethod="bray",
...
```{r hierarchical-cluster-sig, fig.width = 10, fig.height = 4}

hcsample <- get_clust(obj=physeq%>%ps_filter(sample_type=="sigmoid"), distmethod="bray",
                      method="hellinger", hclustmethod="average")
# rectangular layout
cplot1 <- ggclust(obj=hcsample,
                  layout = "rectangular",
