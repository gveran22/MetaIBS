...
path.pozuelo  <- file.path(path.root, "Analysis/Scripts/Individual/Pozuelo-2015")
path.data <- file.path(path.root, "data/phyloseq-objects")
path.plots <- file.path(path.root, "Analysis/Plots/Individual/Pozuelo-2015")
```


## Read data
```{r read-data, echo=FALSE}
physeq <- readRDS(file.path(path.data, "physeq_pozuelo.rds")) # phyloseq object
...
relevant.covariates <- c('Sample', 'Abundance', 'host_disease', 'host_subtype', 'Phylum', 'Collection', 'host_ID')
...
ratio.FB <- left_join(x=bacter, y=firmi, by=c('Sample', 'host_disease', 'host_subtype', 'Collection', 'host_ID')) %>%
