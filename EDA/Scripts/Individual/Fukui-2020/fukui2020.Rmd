...
path.fukui  <- file.path(path.root, "Analysis/Scripts/Individual/Fukui-2020")
path.data <- file.path(path.root, "data/phyloseq-objects")
path.plots <- file.path(path.root, "Analysis/Plots/Individual/Fukui-2020")
```


## Read data
```{r read-data, echo=FALSE}
physeq <- readRDS(file.path(path.data, "physeq_fukui.rds")) # phyloseq object
```
...
# Getting samples with less than 10000 reads
```{r extra analysis, echo=FALSE}
...
```{r looking at taxa 1, echo=FALSE}

asv <- c(otu["SRR11944160",][,otu["SRR11944160",]>0]%>%colnames())

physeq@tax_table[rownames(physeq@tax_table) %in% asv]

```
```{r looking at taxa 2, echo=FALSE}

asv2 <- c(otu["SRR11944177",][,otu["SRR11944177",]>0]%>%colnames())

physeq@tax_table[rownames(physeq@tax_table) %in% asv2]
