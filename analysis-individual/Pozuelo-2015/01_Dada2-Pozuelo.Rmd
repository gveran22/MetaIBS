...
path <- "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015"
...
# out process
out1 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/out1.rds")
out2 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/out2.rds")
raw_stats <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/raw_stats.rds")
# look at quality
...
# error rates
err <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/err.rds")
# infered sequences
seq_infered <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/seq_infered.rds")
# taxa
taxa <- readRDS("/Users/scarcy/Desktop/Praktikum/Data/OutputTaxa/taxa_pozuelo.rds")
# phyloseq object
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/physeq_pozuelo.rds")
```
...
We can see that the reads length has quite reduced (from ~300bp to ~250bp). Looking at the fastq files, the reverse primer was not at the very end of the reads, it was followed by the Golay barcode and other specific sequences described in the methods of the article (we can notably recognize the reverse complement of the full specific sequences 5’-{CAAGCAGAAGACGGCATACGAGAT} {Golay barcode} {AGTCAGTCAGCC} {reverse primer}-3').
...
# Place filtered files in a filtered/ subdirectory
filt2_samples <- file.path(path, "filtered2", paste0(sample.names, "_filt2.fastq.gz"))
# Assign names for the filtered fastq.gz files
...
# Iterate through the 290 samples
...
# We should have 290 samples (290 rows)
dim(seqtable)
...
nb.reads <- data.frame("before" = raw_stats$reads,
                       ...
                       "dataset" = "Pozuelo")
saveRDS(nb.reads, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/DADA2-FILT/seqdepth_pozuelo.rds")
```
...
taxa.print[taxa.print[,1] == 'Eukaryota',2] # the eukaryota are either Euglenozoa, or unknown
...
# Import metadata
metadata_table <- read.table("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/PozueloSraRunTable.txt", header = TRUE, sep = ",")

# Keep relevant covariates
sampledf <- metadata_table[, c("Run", "Sample.Name", "Collection_Date")]
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(out1, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/out1.rds")
saveRDS(out2, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/out2.rds")
saveRDS(err, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/err.rds")
saveRDS(seq_infered, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/seq_infered.rds")


saveRDS(otu_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/ASVtable_final.rds") # ASV table
saveRDS(tax_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/taxa_final.rds") # taxa table
saveRDS(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/metadata_final.rds") # metadata table
write.csv(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/Rproject_Dada2/PozueloMetadata.csv")

saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/physeq_pozuelo.rds") # phyloseq object


# Save nb of reads per sample before / after dada2 pipeline
nb.reads <- readRDS("/Users/scarcy/Desktop/Praktikum/Data/nb_reads.rds")
nb.reads[1:290, 'Pozuelo_before'] <- fastqq(FNs)@nReads
nb.reads[1:290, 'Pozuelo_after'] <- rowSums(seqtable.nochim)
median(na.omit(nb.reads$Pozuelo_before)) # sanity check
median(na.omit(nb.reads$Pozuelo_after)) # sanity check
