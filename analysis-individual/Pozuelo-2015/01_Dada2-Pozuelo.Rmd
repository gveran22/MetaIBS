...
# out process
out1 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/01_Dada2-Pozuelo/out1.rds")
out2 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/01_Dada2-Pozuelo/out2.rds")
raw_stats <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/01_Dada2-Pozuelo/raw_stats.rds")
# look at quality
...
# error rates
err <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/01_Dada2-Pozuelo/err.rds")
# infered sequences
seq_infered <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/01_Dada2-Pozuelo/seq_infered.rds")
# taxa
taxa <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/Taxonomy/output/taxa_pozuelo.rds")
# phyloseq object
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/PhyloTree/input/physeq_pozuelo.rds")
```
...
# Import metadata to remove the 17 samples that are not HC/IBS
metadata_table <- read.csv("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Pozuelo-2015/00_Metadata-Pozuelo/Pozuelo-Metadata.csv", header = TRUE, sep = ",") %>% select(-X)
rownames(metadata_table) <- metadata_table$Run
samples_to_remove <- rownames(metadata_table[metadata_table$host_disease == "TO-DISCARD",])
# length(samples_to_remove) # 17
...
# Remove the samples that are neither healthy nor IBS
nb.reads <- nb.reads[!rownames(nb.reads) %in% samples_to_remove,]
# dim(nb.reads) # 273

# Sanity check
# test <- merge(nb.reads, metadata_table, by="row.names")
# table(test$host_disease) # 88 HC and 185 IBS
...
# Remove samples that are not IBS or healthy
physeq <- subset_samples(physeq, host_subtype != "TO-DISCARD") # n=273
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(raw_stats, file.path(path, "01_Dada2-Pozuelo/raw_stats.rds"))
saveRDS(out1, file.path(path, "01_Dada2-Pozuelo/out1.rds"))
saveRDS(out2, file.path(path, "01_Dada2-Pozuelo/out2.rds"))
saveRDS(err, file.path(path, "01_Dada2-Pozuelo/err.rds"))
saveRDS(seq_infered, file.path(path, "01_Dada2-Pozuelo/seq_infered.rds"))

saveRDS(otu_table(physeq), file.path(path, "01_Dada2-Pozuelo/ASVtable_final.rds")) # ASV table
saveRDS(tax_table(physeq), file.path(path, "01_Dada2-Pozuelo/taxa_final.rds")) # taxa table
saveRDS(sample_data(physeq), file.path(path, "01_Dada2-Pozuelo/metadata_final.rds")) # metadata table

# Taxa & Phyloseq object
saveRDS(taxa, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/Taxonomy/output/taxa_pozuelo.rds")
saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/PhyloTree/input/physeq_pozuelo.rds")
```
