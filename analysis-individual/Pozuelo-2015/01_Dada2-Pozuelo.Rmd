...
SEQBegin <- "TACGKA" # sequence at the beginning of all reads (what is it?!)
...
SEQBegin.orients <- allOrients(SEQBegin)
...
for (i in 1:5){
  ...
  x <-  rbind(ForwardPrimer = sapply(FWD.orients, primerHits, fn = FNs[[i]]), 
              ...
              SeqBeginning = sapply(SEQBegin.orients, primerHits, fn = FNs[[i]]))
  print(x)
...
The reads indeed contain the reverse primer. We could not find the forward primer, but we need to give a _primer.fwd_ parameter to the function 'removePrimers', so we will give the 'TACGKA' sequence that we found at the beginning of all the reads.
We will keep only reads containing the reverse primer, and then remove the primer!
...
out1 <- removePrimers(fn = FNs, fout = filt1_samples,
                      primer.fwd = 'TACGKA', # sequence found at the beginning of all reads
                      primer.rev = REV.orients['RevComp'], # revcomp of the reverse primer, found at the end of the reads
                      trim.fwd = TRUE,
                      trim.rev = TRUE,
