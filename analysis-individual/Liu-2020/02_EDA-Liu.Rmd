...
# Plot Phylum
plot_bar(physeq.liu, fill = "Phylum") + facet_wrap("host_disease", scales="free") +
  theme(axis.text.x = element_text(size = 8))+
  labs(x = "Samples", y = "Absolute abundance", title = "Liu dataset (2020)")

# Plot Class
plot_bar(physeq.liu, fill = "Class")+ facet_wrap("host_disease", scales="free") +
  theme(axis.text.x = element_text(size = 8))+
  labs(x = "Samples", y = "Absolute abundance", title = "Liu dataset (2020)")
```
...
# If I keep only ASVs that are present in at least 2 samples:
physeq.filt <- filter_taxa(physeq.liu, function(x) sum(x > 0) >= 2, TRUE)
phylum.filt <- physeq.filt %>%
  tax_glom(taxrank = "Phylum") %>%
  transform_sample_counts(function(x) {x/sum(x)} ) %>%
  psmelt()
class.filt <- physeq.filt %>%
  tax_glom(taxrank = "Class") %>%
  transform_sample_counts(function(x) {x/sum(x)} ) %>%
  psmelt()
# Plot
ggplot(phylum.filt, aes(x = Sample, y = Abundance, fill = Phylum))+
  facet_wrap(~ host_disease, scales = "free") + # scales = "free" removes empty lines
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(size = 8, angle = -90))+
  labs(x = "Samples", y = "Relative abundance", title = "Liu dataset (2020)")
ggplot(class.filt, aes(x = Sample, y = Abundance, fill = Class))+
  facet_wrap(~ host_disease, scales = "free") + # scales = "free" removes empty lines
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(size = 8, angle = -90))+
  labs(x = "Samples", y = "Relative abundance", title = "Liu dataset (2020)")
...
bacter <- phylum.table %>%
  ...
  select(c('Sample', 'Abundance', 'host_disease', 'Phylum', 'Bristol', 'IBS_SSS')) %>%
  arrange(Sample)
...
ratio.FB <- data.frame('Sample' = bacter$Sample,
                       ...
                       'Bristol' = bacter$Bristol,
                       'IBS_SSS' = bacter$IBS_SSS,
...
ggplot(ratio.FB, aes(x = Bristol, y = logRatioFB))+
  geom_boxplot(outlier.shape = NA, aes(group=Bristol))+
  geom_jitter(width=0.1)+
  labs(x = "Bristol scale",  y = 'Log2(Firmicutes/Bacteroidota)', title = "Firmicutes:Bacteroidota ratio")+
  theme_cowplot()

# Plot by IBS symptom severity score (mild IBS < 175; moderate < 300; severe >300)
ggplot(ratio.FB, aes(x = IBS_SSS, y = logRatioFB))+
  geom_point()+
  labs(x = "IBS SSS",  y = 'Log2(Firmicutes/Bacteroidota)', title = "Firmicutes:Bacteroidota ratio")+
  theme_cowplot()
```
...
matcol <- data.frame(group = sample_data(physeq.liu)[,"host_disease"])
...
plotHeatmaps <- function(dlist, fontsize){
  ...
  for(d in dlist){
    plist[[i]] <- pheatmap(as.matrix(d), 
                          ...
                          annotation_colors = list(host_disease = c('Healthy' = 'blue', 'IBS-D' = 'red')),
                          cluster_rows = T,
                          cluster_cols = T,
