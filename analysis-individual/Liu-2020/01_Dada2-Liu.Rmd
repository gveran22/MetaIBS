...
#_____________________________________________________________________
path <- "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu"

# seq depth
raw_stats <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/raw_stats.rds")
# out process
out2 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/out2.rds")
# quality profiles
filt2_samples <- sort(list.files(file.path(path, "filtered2"), pattern=".fastq.gz", full.names = TRUE))
# error rates
err <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/err.rds")
# infered ASVs
seq_infered <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/seq_infered.rds")
# taxa
taxa <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/OutputTaxa/taxa_Liu.rds")
# phyloseq object
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/physeq_Liu.rds")
```
...
First, we import the fastq files containing the raw reads. The samples were downloaded from the SRA database with the accession number PRJEB11419. IBS and healthy samples were selected out of the thousands of samples as described in [00_Metadata-Liu.R](00_Metadata-Liu.R).
...
metadata <- read.csv(file.path(path, "Metadata-Liu.csv"))
...
out1 <- removePrimers(fn = FNs, fout = filt1_samples,
                      primer.fwd = 'GTGRGGAATATT', # sequence found at the beginning of all reads
...
out2 <- filterAndTrim(fwd = FNs, filt = filt_test, # filt = filt2_samples,
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(raw_stats, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/raw_stats.rds")
saveRDS(out2, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/out2.rds")
saveRDS(err, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/err.rds")
saveRDS(seq_infered, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/seq_infered.rds")


saveRDS(otu_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/ASVtable_final.rds") # ASV table
saveRDS(tax_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/taxa_final.rds") # taxa table
saveRDS(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/metadata_final.rds") # metadata table
write.csv(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/analysis-individual/Liu/Metadata-Liu.csv")

saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/physeq_Liu.rds") # phyloseq object
saveRDS(physeq.filt, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu/01_Dada2-Liu/physeq(ASVfilt)_Liu.rds")
...
nb.reads[1:1191, 'Liu_before'] <- fastqq(FNs)@nReads
nb.reads[1:1191, 'Liu_after'] <- rowSums(seqtable.nochim)
median(na.omit(nb.reads$Liu_before)) # sanity check
median(na.omit(nb.reads$Liu_after)) # sanity check
