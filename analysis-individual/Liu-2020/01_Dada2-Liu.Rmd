...
# phyloseq object
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu-2020/01_Dada2-Liu/physeq_Liu.rds")
```
...
write.csv(sample_data(physeq), file.path(path, "Metadata-Liu.csv"))

saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/phyloseq-objects/physeq_liu.rds") # phyloseq object
```
...
```{r plot, fig.width = 10}
# Absolute abundance
plot_bar(physeq.filt, fill = "Phylum")+ facet_wrap("host_disease", scales="free") + theme(axis.text.x = element_blank()) # long to compute

# Relative abundance for Phylum
phylum.table <- physeq.filt %>%
  tax_glom(taxrank = "Phylum") %>%                     # agglomerate at phylum level
  transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance
  psmelt()                                             # Melt to long format
