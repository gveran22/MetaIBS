...
physeq.rel <- readRDS(file.path(path, "data/analysis-individual/Liu-2020/02_EDA-Liu/physeq_relative.rds"))
physeq.clr <- readRDS(file.path(path, "data/analysis-individual/Liu-2020/02_EDA-Liu/physeq_clr.rds"))
physeq.CSN <- readRDS(file.path(path, "data/analysis-individual/Liu-2020/02_EDA-Liu/physeq_CSN.rds"))
physeq.NZcomp <- readRDS(file.path(path, "data/analysis-individual/Liu-2020/02_EDA-Liu/physeq_NZcomp.rds"))
```
...
plot_bar(physeq.liu, fill = "Phylum") + facet_wrap("host_subtype", scales="free_x") +
  ...
  guides(fill=guide_legend(ncol=2))+
  labs(x = "Samples", y = "Total read count", title = "Liu dataset (2020)")
dev.off()
...
ggplot(ratio.FB, aes(x = host_subtype, y = logRatioFB))+
  geom_violin(lwd=3, aes(fill=host_subtype))+
  geom_jitter(size = 8, width=0.1)+
  scale_fill_manual(values=scales::alpha(c("blue", "red"), .3))+
  labs(x = "",  y = 'Log2(Firmicutes/Bacteroidota)', title = "Liu dataset (2020)")+
  theme_cowplot()+
...
# Firmicutes:Bacteroidota log ratio by Bristol scale
jpeg(file.path(path.plots, "ratioFB_Bristol.jpg"), width = 1500, height = 2000)
ggplot(ratio.FB, aes(x = Bristol, y = logRatioFB))+
  geom_violin(aes(group=Bristol), lwd=3)+
  geom_jitter(aes(color=host_subtype), width=0.1, size=8)+
  scale_color_manual(values=c("blue", "red"))+
  labs(x = "Bristol stool scale",  y = 'Log2(Firmicutes/Bacteroidota)', title = "Liu dataset (2020)")+
  theme_cowplot()+
  theme(axis.text = element_text(size = 50),
        axis.title = element_text(size = 50),
        title = element_text(size = 50),
...
# Firmicutes:Bacteroidota log ratio by IBS symptom severity score
jpeg(file.path(path.plots, "ratioFB_IBS-SSS.jpg"), width = 1500, height = 2000)
ggplot(ratio.FB, aes(x = IBS_SSS, y = logRatioFB))+
  geom_point(aes(color=host_subtype), size=8)+
  scale_color_manual(values=c("blue", "red"))+
  labs(x = "IBS SSS",  y = 'Log2(Firmicutes/Bacteroidota)')+
  theme_cowplot()+
  theme(axis.text = element_text(size = 50),
...
plot_richness(physeq.liu, x="host_subtype", measures="Simpson", color="host_subtype") +
  ...
  labs(x="", y="Simpson")
```
![(A-C) Simpson, Ace and Chao index of IBS-D and HC groups. Welch's test. Means ± SD](/Users/scarcy/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu-2020/02_EDA-Liu/fig1A.png){width=30%}
...
```
![(D) PCoA of fecal microbiota composition in IBS-D and HC groups based on bray-curtis distance.](/Users/scarcy/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu-2020/02_EDA-Liu/fig1D.png){width=40%}
...
![(E) Hierarchical clustering of fecal microbiota composition in IBS-D and HC groups based on bray-curtis distance.](/Users/scarcy/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu-2020/02_EDA-Liu/fig1E.png){width=30%}
...
![(A) Relative taxonomic abundances at the level of phylum are shown in bar charts.](/Users/scarcy/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Liu-2020/02_EDA-Liu/fig2A.png){width=40%}
