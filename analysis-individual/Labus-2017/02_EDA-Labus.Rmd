...
knitr::opts_chunk$set(fig.width = 15, fig.height = 5, warning=FALSE, message=FALSE,
                      root.dir = "/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/")
```
...
# Import metadata
metadata_table <- read.csv("/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/LabusMetadata.csv")
...
metadata_table$host_subtype[metadata_table$host_subtype == "IBS-A"] <- "IBS-M"
colnames(metadata_table) <- c("Run", "host_disease", "host_subtype", "host_age", "host_sex", "host_bmi")
metadata_table$author <- "Labus"
metadata_table$sequencing_tech <- "454 pyrosequencing"
metadata_table$variable_region <- "V3-V5"
saveRDS(metadata_table, "/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/metadata_final.rds")

# Import OTU table (rows: sample names // columns: sequence variants)
seqtable.nochim <- readRDS("/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/ASVtable_final.rds")
...
taxa <- readRDS("/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/taxa_final.rds")
dim(taxa)
```
XXXXXXX
...
cat("There are", nrow(metadata[metadata$host_disease=="IBS", ]), "individuals with IBS and",
    nrow(metadata[metadata$host_disease=="Healthy", ]), "healthy individuals.")

# Age
mean(metadata[metadata$host_disease == "IBS",]$host_age)
sd(metadata[metadata$host_disease == "IBS", ]$host_age)

mean(metadata[metadata$host_disease == "Healthy",]$host_age)
sd(metadata[metadata$host_disease == "Healthy", ]$host_age)

wilcox.test(metadata[metadata$host_disease == "IBS", ]$host_age,
            metadata[metadata$host_disease == "Healthy", ]$host_age)

# Gender
table(metadata[metadata$host_disease == "IBS", ]$host_sex) # 21 females / 29 total
table(metadata[metadata$host_disease == "Healthy", ]$host_sex) # 14 females / 29 total
chisq.test(data.frame("Female" = c(21,14),
                      "Male" = c(8,9)))

# BMI
mean(metadata[metadata$host_disease == "IBS",]$host_bmi, na.rm = TRUE)
sd(metadata[metadata$host_disease == "IBS", ]$host_bmi, na.rm = TRUE)

mean(metadata[metadata$host_disease == "Healthy",]$host_bmi, na.rm = TRUE)
sd(metadata[metadata$host_disease == "Healthy",]$host_bmi, na.rm = TRUE)
...
Phylogenetic trees are needed to measure phylogeny-based distances between microbial communities. We will construct the phylogenetic tree following the protocol in the paper _Bioconductor Workflow for Microbiome Data Analysis: from raw reads to community analyses_ (https://f1000research.com/articles/5-1492).
...
# Save physeq object
saveRDS(physeq, "/Users/scarcy/Desktop/Praktikum/Data/Labus_2017/Rproject_DataAnalysis/physeq.rds")
saveRDS(physeq, "/Users/scarcy/Desktop/Praktikum/Data/Rproject_COMBINE/physeq_labus.rds")
...
ggplot(ratio_FB, aes(x = host_disease, y = Log_ratio_FB))+
  ...
  labs(x = "",  y = 'Log2 Firmicutes/Bacteroidota Abundance Ratio', title = "Labus dataset (2017)")+
  theme_classic()+
