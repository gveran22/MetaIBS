...
path <- "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017"
...
# nb of reads after filtering steps (out variables)
out1 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/out1.rds")
out2 <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/out2.rds")
# samples after primer removal / quality filtering
...
# error rates
err <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/err.rds")
# inferred ASVs
seq_infered <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/seq_infered.rds")
# taxa
taxa <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/OutputTaxa/taxa_labus.rds")
#physeq
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/phyloseq-objects/physeq_labus.rds")
```
...
out1 <- removePrimers(fn = FNs, fout = filt1_samples,
                      primer.fwd = REV, # reverse primer is found at the beginning of the reads
                      trim.fwd = TRUE,
...
# Sanity check if we still find the primers in the reads
for (i in 1:4){
  cat("SAMPLE", sample.names[i], "with total number of", raw_stats[i,'reads'], "reads\n\n")
  # Get a table to know how many times the 357F and 926R primers are found (in how many reads)
  x <-  rbind(ForwardPrimer = sapply(FWD.orients, primerHits, fn = filt1_samples[[i]]), 
              ReversePrimer = sapply(REV.orients, primerHits, fn = filt1_samples[[i]]))
  print(x)
  cat("\n____________________________________________\n\n")
}
```
...
# Look at number of reads per sample
#raw_stats_filt <- data.frame('sample' = sample.names,
#                             'reads' = fastqq(filt2_samples)@nReads)
#min(raw_stats_filt$reads)
#max(raw_stats_filt$reads)
#mean(raw_stats_filt$reads)
```
...
# Import metadata
metadata_table <- read.csv("~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/LabusMetadata.csv")

# Keep relevant covariates
sampledf <- metadata_table[, c("Run", "host_disease", "host_subtype", "Host_Age", "host_sex", "host_body_mass_index", "Collection_Date")]
rownames(sampledf) <- metadata_table$Run # assign sample names as row names
colnames(sampledf) <- c("Run", "host_disease", "host_subtype", "host_age", "host_sex", "host_bmi", "collection_date")

sampledf$host_disease <- as.character(sampledf$host_disease)

# Simplify data
sampledf$host_disease[sampledf$host_disease == "None"] <- "Healthy"
sampledf$host_disease[sampledf$host_disease == "Irritable bowel syndrome"] <- "IBS"
sampledf$host_subtype[sampledf$host_subtype == "IBS-A"] <- "IBS-M"

sampledf$author <- "Labus"
sampledf$sequencing_tech <- "454 pyrosequencing"
sampledf$variable_region <- "V3-V5"
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(out1, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/out1.rds")
saveRDS(out2, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/out2.rds")
saveRDS(err, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/err.rds")
saveRDS(seq_infered, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/seq_infered.rds")


saveRDS(otu_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/ASVtable_final.rds") # ASV table
saveRDS(tax_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/taxa_final.rds") # taxa table
saveRDS(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/metadata_final.rds") # metadata table
write.csv(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Labus-2017/01_Dada2-Labus/LabusMetadata.csv") # metadata table

saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/phyloseq-objects/physeq_labus.rds") # phyloseq object


# Save nb of reads per sample before / after dada2 pipeline
nb.reads <- data.frame('Labus_before' = fastqq(FNs)@nReads,
                       'Labus_after' = rowSums(seqtable.nochim))
