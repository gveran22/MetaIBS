...
for (i in 1:607){
  ...
  tryCatch({
    out <- NULL # initialize
    out <- filterAndTrim(fwd = FWD.filt1_samples[i], filt = FWD.filt_test[i], # filt = FWD.filt2_samples[i],
                        rev = REV.filt1_samples[i], filt.rev = REV.filt_test[i], # filt.rev = REV.filt2_samples[i],
