...
To keep the original SRA metadata table "intact", we will duplicate it. We will rename a few diseases to avoid confusions (i.e Hyperplastic_polyp & hyperplastic_polyp). Also, as for some samples the day of the collection date is not always provided, we will create a new column to store the information of year&month of collection date.
...
More than 100 samples with same host_ID have different disease phenotype.

```{r metadata-inconsistencies-1}
...
Now, if we were to consider other covariates as proxies for sample identifiers, we get much less inconsistencies in disease phenotype. We can use BMI, age and collection date as relevant information to identify samples belonging to the same individual.

```{r metadata-inconsistencies-2}
...
```
Five samples have same collection_date, 
