...
```{r sra-metadata}
########
# Import the metadata table downloaded from the SRA
sraDF <- read.csv("../../SraRunTable.txt", header=TRUE, sep = ",")
head(sraDF)
...
# Do a few modifications for simplicity
metadata <- metadata[metadata$environment_.biome. != "bioreactor",] # remove "bioreactor" samples
metadata$environment_.biome. <- NULL

metadata <- transform(metadata, host_body_product = as.character(host_body_product)) # host_body_product as character
metadata <- transform(metadata, gastrointestinal_tract_disorder = as.character(gastrointestinal_tract_disorder))
metadata[metadata$host_body_product == "Mucosal tissue","host_body_product"] <- "sigmoid"
metadata[metadata$host_body_product == "Faeces","host_body_product"] <- "stool"

colnames(metadata) <- c("Run", "collection_date", "sample_type", "host_age",
                        "host_bmi", "host_ID", "sample_storage_duration",
                        "sequencing_run", "extraction_plate", "host_psy",
                        "host_sex", "host_disease")

metadata <- transform(metadata, host_psy = as.character(host_psy))
metadata <- transform(metadata, host_sex = as.character(host_sex))
metadata <- transform(metadata, collection_date = as.character(collection_date))
metadata$host_disease <- gsub("IBS.*", "IBS", metadata$host_disease)
