...
metadata %>%
  ...
  # mutate(test=case_when(new_sex != host_sex ~ "diff",
  #                          TRUE ~ NA_character_)) %>%
  # group_by(Run) %>%
  # filter(new_sex != host_sex) %>%
  # Change column order
  relocate(collection_date) %>%
  relocate(Run, .after=extraction_plate) %>%
  relocate(host_psy, .after=host_ID) %>%
  relocate(new_id, .after=host_disease) %>%
  relocate(host_ID, .after=new_id) %>%
  relocate(new_sex, .after=host_sex) %>%
  relocate(test, .after=new_sex)
  #group_vars()
```
