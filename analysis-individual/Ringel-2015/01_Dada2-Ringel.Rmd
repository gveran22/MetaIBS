...
# taxa
taxa <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/OutputTaxa/taxa_ringel.rds")
#physeq
physeq <- readRDS("~/Projects/IBS_Meta-analysis_16S/phyloseq-objects/physeq_ringel.rds")
```
...
FWD.orients <- allOrients(FWD) # get all combinations for the 8FM primer sequence
...
out1 <- removePrimers(fn = FNs, fout = filt1_samples,
                      primer.fwd = 'TTTGATCMTGGCTCAG', # forward primer is found at the beginning of the reads
                      primer.rev = 'TACGGRAGGCAGCAG', # revcomp of the REV primer at the end of the reads
                      trim.fwd = TRUE,
                      trim.rev = TRUE,
...
# Filter
out2 <- filterAndTrim(fwd = filt1_samples, filt = filt_test, # filt = filt2_samples,
...
Considering that both primers were found in the reads, and that the V1-V2 region should not be larger than ~350bp, we will remove any sequence variant above that 400bp, as they could be due to non-specific priming.

```{r new-seq-table, fig.height=3, fig.width=4}
# Remove any sequence variant below 400bp
seqtable_new <- seqtable[,nchar(colnames(seqtable)) %in% 300:400]

# We should have 76 samples (76 rows)
dim(seqtable_new)

# Inspect distribution of sequence lengths
hist(nchar(getSequences(seqtable_new)), breaks = 100, xlab = "ASV length", ylab = "Number of ASVs", main="")
```
...
```{r remove-chimeras}
seqtable.nochim <- removeBimeraDenovo(seqtable_new, method="consensus", multithread=TRUE, verbose=TRUE)
...
sum(seqtable.nochim)/sum(seqtable_new)
```
...
```{r save, eval=FALSE}
# Save to disk
saveRDS(out1, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/out1.rds")
saveRDS(out2, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/out2.rds")
saveRDS(err, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/err.rds")
saveRDS(seq_infered, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/seq_infered.rds")

saveRDS(otu_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/ASVtable_final.rds") # ASV table
saveRDS(tax_table(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/taxa_final.rds") # taxa table
saveRDS(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/metadata_final.rds") # metadata table
write.csv(sample_data(physeq), "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/Ringel-2015/01_Dada2-Ringel/RingelMetadata.csv")

saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/phyloseq-objects/physeq_ringel.rds") # phyloseq object


# Save nb of reads per sample before / after dada2 pipeline
nb.reads <- readRDS("~/Projects/IBS_Meta-analysis_16S/data/nb_reads.rds")
nb.reads[1:76, 'Ringel_before'] <- fastqq(FNs)@nReads
nb.reads[1:76, 'Ringel_after'] <- rowSums(seqtable.nochim)
median(na.omit(nb.reads$Ringel_before)) # sanity check
median(na.omit(nb.reads$Ringel_after)) # sanity check
