...
# Save to disk
saveRDS(raw_stats, file.path(path, "01_Dada2-Fukui/raw_stats.rds"))
saveRDS(out2, file.path(path, "01_Dada2-Fukui/out2.rds"))
saveRDS(errF, file.path(path, "01_Dada2-Fukui/errF.rds"))
saveRDS(errR, file.path(path, "01_Dada2-Fukui/errR.rds"))
saveRDS(dadaFs, file.path(path, "01_Dada2-Fukui/infered_seq_F.rds"))
saveRDS(dadaRs, file.path(path, "01_Dada2-Fukui/infered_seq_R.rds"))
saveRDS(mergers, file.path(path, "01_Dada2-Fukui/mergers.rds"))

# Taxa
saveRDS(taxa, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/Taxonomy/output/taxa_fukui.rds")
```
...
non0_ASVs <- apply(otu_table(physeq)["SRR11944160",], 2, function(col) all(col !=0 ))
otu_table(physeq)["SRR11944160",non0_ASVs]

# There are only 3 ASVs in this sample, so we will remove it.
physeq <- subset_samples(physeq, Run != "SRR11944160")

# Discard any ASV that is present in no sample anymore
physeq <- filter_taxa(physeq, function(x) sum(x > 0) > 0, TRUE) # 0 taxa discarded


# Save the ASV, taxa, metadata tables
# saveRDS(otu_table(physeq), file.path(path, "01_Dada2-Fukui/ASVtable_final.rds")) # ASV table
# saveRDS(tax_table(physeq), file.path(path, "01_Dada2-Fukui/taxa_final.rds")) # taxa table
# saveRDS(sample_data(physeq), file.path(path, "01_Dada2-Fukui/metadata_final.rds")) # metadata table

# Save the phyloseq object
# saveRDS(physeq, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/CLUSTER/PhyloTree/input/physeq_fukui.rds")
```
