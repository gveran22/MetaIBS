...
out2[3:6,] # show how many reads were filtered in each file
...
for(sampl in sample.names) {
  ...
  seq_infered[[sampl]] <- dada(derep, err=err, multithread=TRUE,
...
# Sequences should be between 28F - 519R, so around ~450bp (removing the primers lengths).
# We kept reads that contain the forward primer, so V3-V5 regions should not be much longer than 450bp
# Remove any sequence variant beyond 500bp
seqtable.new <- seqtable[,nchar(colnames(seqtable)) <= 500]
dim(seqtable.new)
hist(nchar(getSequences(seqtable.new)), breaks = 100, xlab = "Read length", ylab = "Number of reads", main="")
```
...
nb.reads <- data.frame("before" = raw_stats$reads,
                       ...
                       "dataset" = "LoPresti")
saveRDS(nb.reads, "~/Projects/IBS_Meta-analysis_16S/data/analysis-individual/DADA2-FILT/seqdepth_lopresti.rds")
```
